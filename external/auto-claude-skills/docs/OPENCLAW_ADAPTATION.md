...
- Workflow 2：实验执行 → ULTRAWORK 回环（Oracle 验证）→ 迭代修复
...
| `/ulw-loop` | ULTRAWORK 回环（Oracle 验证 + 证据记录） | `outputs/ulw_loop.md` |
...
### 阶段4：ULTRAWORK 回环
```text
执行阶段4：开始 ULTRAWORK，建议 --max-iterations=4；每轮记录证据、阻塞点与修复动作，更新 outputs/ulw_loop.md。
```
...
2. **轮次控制**：ULTRAWORK 建议设置 max-iterations=4，避免无穷迭代。  
