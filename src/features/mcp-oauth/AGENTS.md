...
18 files. Full OAuth 2.0 authorization flow for MCP servers requiring authentication. Implements PKCE (RFC 7636), Dynamic Client Registration (DCR, RFC 7591), and resource indicators (RFC 8707). Used by `bunx openagent-labforge mcp-oauth login`.
...
```bash
bunx openagent-labforge mcp-oauth login <server-url>   # Full PKCE flow
bunx openagent-labforge mcp-oauth logout <server-url>  # Revoke + delete token
bunx openagent-labforge mcp-oauth status               # List stored tokens
```
