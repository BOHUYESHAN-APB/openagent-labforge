...
describe("MOMUS_SYSTEM_PROMPT policy requirements", () => {
  ...
  test("should extract paths containing .opencode/openagent-labforge/plans/ and ending in .md", () => {
    // given
    const prompt = MOMUS_SYSTEM_PROMPT

    // when / #then
    expect(prompt).toContain(".opencode/openagent-labforge/plans/")
  ...
  test("should NOT teach that 'Please review' is INVALID (conversational wrapper allowed)", () => {
    ...
    const invalidExample = "Please review .opencode/openagent-labforge/plans/plan.md"
