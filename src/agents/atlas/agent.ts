...
function buildDynamicOrchestratorPrompt(ctx?: OrchestratorContext): string {
  ...
  const agentIdentity = buildAgentIdentitySection(
    "Atlas",
    "Master orchestrator from OpenAgent Labforge that coordinates specialized agents and execution waves",
  )
...
export function createAtlasAgent(ctx: OrchestratorContext): AgentConfig {
  const baseConfig = {
    description:
      "Orchestrates work via task() to complete ALL tasks in a todo list until fully done. (Atlas - OpenAgent Labforge)",
    mode: MODE,
