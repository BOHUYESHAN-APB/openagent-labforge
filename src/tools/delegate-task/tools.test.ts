...
describe("sisyphus-task", () => {
  ...
  describe("category delegation config validation", () => {
    ...
    test("proceeds without error when systemDefaultModel is undefined", async () => {
      ...
      const { createDelegateTask } = require("./tools")
       ...
       expect(result).not.toContain("openagent-labforge requires a default model")
    }, { timeout: 10000 })
  ...
  describe("subagent_type model extraction (issue #1225)", () => {
    ...
    test("agentOverrides model takes priority over matchedAgent.model (#1357)", async () => {
      // given - user configured oracle to use a specific model in openagent-labforge.json
      const { createDelegateTask } = require("./tools")
      let promptBody: any
