...
import {
  ...
  type PromptConfigCheckpoint,
} from "../../shared/compaction-agent-config-checkpoint"
import { createInternalAgentTextPart } from "../../shared/internal-initiator-marker"
import { log } from "../../shared/logger"
import { setSessionModel } from "../../shared/session-model-state"
import { setSessionTools } from "../../shared/session-tools-store"
import { createExpectedRecoveryPromptConfig, isPromptConfigRecovered } from "./recovery-prompt-config"
import { resolveLatestSessionPromptConfig, resolveSessionPromptConfig } from "./session-prompt-config-resolver"
import {
...
function checkpointModelToSessionModel(
  model: PromptConfigCheckpoint["model"],
...
export function createRecoveryLogic(
  ...
  const recoverCheckpointedAgentConfig = async (
    ...
    const launchAgent =
      resolveRegisteredAgentName(expectedPromptConfig.agent) ??
      expectedPromptConfig.agent
    ...
    if (reason === "session.compacted") {
      ...
      if (
        isPromptConfigRecovered(latestPromptConfig, expectedPromptConfig)
      ) {
        return false
      }
    }
    ...
    try {
      ...
      const sessionModel = checkpointModelToSessionModel(model)
      if (sessionModel) {
        setSessionModel(sessionID, sessionModel)
      }
      if (tools) {
  ...
  const maybeWarnAboutNoTextTail = async (
    sessionID: string,
  ): Promise<void> => {
    const tailState = getTailState(sessionID)
