...
describe("findPluginEntry", () => {
  ...
  test("returns unpinned for bare package name", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["openagent-labforge"] }))
  ...
  test("returns unpinned for latest dist-tag", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["openagent-labforge@latest"] }))
  ...
  test("returns unpinned for beta dist-tag", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["openagent-labforge@beta"] }))
  ...
  test("returns pinned for explicit semver", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["openagent-labforge@3.5.2"] }))
