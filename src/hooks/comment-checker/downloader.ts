...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[openagent-labforge] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[openagent-labforge] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[openagent-labforge] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[openagent-labforge] Comment checking disabled.`)
    return null
  }
}
