...
function buildCompressionResultPrompt(result: CompressionResult): string {
  const lines = [
    ...
    "",
    "## Command Split",
    "- `/compress-context` manages operational compression for the current session.",
    "- `/checkpoint` writes an explicit durable handoff artifact for later recovery or cross-session continuation.",
