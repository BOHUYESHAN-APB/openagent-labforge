...
describe("getPluginNameWithVersion", () => {
  ...
  test("returns @latest when current version matches latest tag", async () => {
    ...
    expect(result).toBe("openagent-labforge@latest")
  })
  ...
  test("returns @beta when current version matches beta tag", async () => {
    ...
    expect(result).toBe("openagent-labforge@beta")
  })
  ...
  test("returns @next when current version matches next tag", async () => {
    ...
    expect(result).toBe("openagent-labforge@next")
  })
  ...
  test("returns bare package name when npm returns non-ok response for stable version", async () => {
    ...
    expect(result).toBe("openagent-labforge")
  })
...
describe("fetchNpmDistTags", () => {
  ...
  test("returns dist-tags on success", async () => {
    ...
    const result = await fetchNpmDistTags("openagent-labforge")
...
describe("generateOmoConfig - model fallback system", () => {
  ...
  test("uses ultimate fallback when no providers configured", () => {
    ...
    expect(result.$schema).toBe("https://raw.githubusercontent.com/code-yeongyu/openagent-labforge/dev/assets/openagent-labforge.schema.json")
