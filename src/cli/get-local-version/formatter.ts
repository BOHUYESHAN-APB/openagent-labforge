...
export function formatVersionOutput(info: VersionInfo): string {
  ...
  lines.push("")
  lines.push(color.bold(color.white("openagent-labforge Version Information")))
  ...
  switch (info.status) {
    ...
    case "outdated":
      ...
      lines.push(`  ${color.dim("Run:")} ${color.cyan("cd ~/.config/opencode && bun update @bohuyeshan/openagent-labforge-core")}`)
      break
    case "local-dev":
