...
> **翻訳同期について**
>
> この日本語 README は主要な実務情報を追従しますが、全文同期は英語版
> `README.md` と簡体字中国語版 `README.zh-cn.md` が先行する場合があります。
> 差異がある場合は英語版と中国語版を優先してください。
...
安定した結果のため、**400K を超えるコンテキストを安定提供できるモデル**
を優先してください。

実務上の目安:

- 400K 超のモデルは短いコンテキストのモデルよりかなり安定しやすい
- 実効的な作業コンテキストは 500K から 550K 前後を目安に保つ
- 宣伝上の最大値まで常に使い切る前提では運用しない
- 長時間の自動実行、生物情報学の長いセッション、深い実装セッションでは
  **500K 超**を強く推奨
- **1M 以上**を安定提供できるモデルが最も扱いやすい

理由:

- モデル自身が大きな summary を生成することがある
- compaction の間隔が非常に短くなることがある
- 同じモデル名でも provider ごとに実効コンテキストがかなり異なる

したがって、モデル名だけではなく provider ごとの実際の上限を見るのが安全です。
