...
manifest 当前记录生成的 commands、生成的 skills 和 backups。后续它还会扩展到 MCP snippets、hook snippets、docs 以及旧版本遗留文件。
...
如果用户改过文件，默认保留并打印警告。当前已经实现 `--force`，但在删除前应先写入 backup。

如果存在被保留的受管文件，manifest 也应保留。这样后续清理时不会丢失“哪些文件仍然归 LabForge 管”的信息。
