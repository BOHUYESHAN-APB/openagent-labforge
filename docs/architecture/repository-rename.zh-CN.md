...
这个仓库名现在是历史遗留。产品和 npm package 名正在迁移到 `extendai-lab`，未来 GitHub 仓库也应迁移到这个中性名称。
...
> 当前 GitHub 仓库仍叫 `openagent-labforge-bio`，这是为了历史 release 连续性。产品/package 名是 `extendai-lab`；bio 是第一个 discipline pack，不是产品边界。
