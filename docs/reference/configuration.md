...
Complete reference for `openagent-labforge.jsonc` configuration. This document covers every available option with examples.
...
1. `.opencode/openagent-labforge.jsonc` / `.opencode/openagent-labforge.json`
...
Run `bunx openagent-labforge install` for guided setup. Run `opencode models` to list available models.
...
Run `bunx openagent-labforge doctor --verbose` to see effective model resolution for your config.
