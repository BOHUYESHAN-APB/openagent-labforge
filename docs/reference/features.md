# openagent-labforge Features Reference

## Agents

openagent-labforge provides 11 specialized AI agents. Each has distinct expertise, optimized models, and tool permissions.
...
Customize agent models, prompts, and permissions in `openagent-labforge.json`.
...
You can define custom categories in `openagent-labforge.json`.
...
openagent-labforge provides two browser automation providers, configurable via `browser_automation_engine.provider`.
...
**Note on Claude Code Alignment**: This implementation follows Claude Code's internal Task tool signatures (`TaskCreate`, `TaskUpdate`, `TaskList`, `TaskGet`) and field naming conventions (`subject`, `blockedBy`, `blocks`, etc.). However, Anthropic has not published official documentation for these tools. This is OpenAgent Labforge's own implementation based on observed Claude Code behavior and internal specifications.
