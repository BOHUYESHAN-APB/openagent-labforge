...
class BioMatrix {
  constructor(canvasId) {
    ...
    this.bioTerms = [
      'BLAST', 'FASTQ', 'BAM', 'VCF', 'GFF',
      'ORF', 'SNP', 'Indel', 'Reads', 'Align',
      'Phred', 'Contig', 'Scaffold', 'Assembly',
      'Promoter', 'Exon', 'Intron', 'UTR',
      'Genome', 'Transcript', 'Protein', 'Peptide',
      'Helix', 'Primer', 'PCR', 'qPCR',
      'RNA-seq', 'ChIP-seq', 'ATAC', 'Methyl',
      'Codon', 'Anticodon', 'Poly-A', 'cDNA'
    ];
  ...
  update() {
    this.columns.forEach(col => {
      ...
      col.chars = col.chars.filter(c => c.y < this.height + 20 && c.alpha > 0.02);
    });
  }
  ...
  draw() {
    ...
    this.columns.forEach(col => {
      col.chars.forEach(c => {
        ...
        ctx.font = c.isTerm ? 'bold 13px JetBrains Mono' : '13px JetBrains Mono';
