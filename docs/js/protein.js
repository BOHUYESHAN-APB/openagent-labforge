  ...
  loadFromRCSB(pdbId) {
    ...
    fetch(url)
      ...
      .catch(err => {
        ...
        this.container.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#8888aa;">Failed to load protein structure</div>';
      });
  }
    ...
    switch (style) {
      case 'cartoon':
        ...
        Object.entries(chainColors).forEach(([chain, color]) => {
          this.viewer.setStyle({ chain: chain }, { 
            cartoon: { 
              color: color,
              opacity: 0.9
            }
